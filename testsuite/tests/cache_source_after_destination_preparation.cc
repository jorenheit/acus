// Copy from one dynamic array field to another; destination preparation must not corrupt the source.
// Expected: "BBC"

TEST_BEGIN

auto u8 = ts::u8();

auto pairT = ts::defineStruct("Pair")
  .field("x", u8)
  .field("y", u8)
  .done();

auto arrT = ts::array(pairT, 3);

c.function("main").begin(); {
  c.declareLocal("arr", arrT);
  c.declareLocal("i", u8);
  c.declareLocal("j", u8);

  c.assign(c.structField(c.arrayElement("arr", 0), "x"), literal::u8('A'));
  c.assign(c.structField(c.arrayElement("arr", 1), "x"), literal::u8('B'));
  c.assign(c.structField(c.arrayElement("arr", 2), "x"), literal::u8('C'));

  c.assign("i", literal::u8(0));
  c.assign("j", literal::u8(1));

  auto dst = c.structField(c.arrayElement("arr", "i"), "x");
  auto src = c.structField(c.arrayElement("arr", "j"), "x");

  c.assign(dst, src);

  c.write(c.structField(c.arrayElement("arr", 0), "x"));
  c.write(c.structField(c.arrayElement("arr", 1), "x"));
  c.write(c.structField(c.arrayElement("arr", 2), "x"));

  c.returnFromFunction();
} c.endFunction();

TEST_END
